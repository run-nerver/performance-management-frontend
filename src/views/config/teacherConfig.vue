<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="系数" width="110px" align="center">
        <template slot-scope="{ row }">
    ...
    <el-dialog :visible.sync="dialogFormVisible">
      ...
      >
        ...
        <el-form-item label="系数">
          <el-input v-model="temp.coefficient" type="textarea" :rows="1" />
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    getList() {
      ...
      fetchCoefficientPaginate(this.listQuery).then(response => {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          createCoefficient(this.temp).then((response) => {
    ...
    updateData() {
      this.$refs['dataForm'].validate(valid => {
        if (valid) {
          ...
          updateCoefficient(tempData).then((response) => {
    ...
    handleDelete(row, index) {
      ...
      deleteCoefficient(tmpId.id).then(() => {
        this.$notify({
          title: '成功',
          message: '删除成功',
          type: 'success',

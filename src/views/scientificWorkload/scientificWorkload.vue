<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input
        v-model="listQuery.scientificName"
        placeholder="名称"
        style="width: 200px;"
        class="filter-item"
      ...
      />
      <el-input
        v-model="listQuery.scientificType"
        placeholder="类型"
        style="width: 200px;"
        class="filter-item"
        @keyup.enter.native="handleFilter"
...
import {
  UploadExcelScientificWorkload,
  fetchUserInfo
} from '@/api/scientific-workload'
...
export default {
  ...
  methods: {
    ...
    uploadExcel() {
      ...
      UploadExcelScientificWorkload(upData, this.listQuery.uploadExcelYear).then(response => {
    ...
    attDownload() {
      ...
      var path = url + '?filename=' + '科研工作量导入模板.xls'
      axios.post(path, {}, { responseType: 'arraybuffer' }).then((res) => {
        fileDownload(res.data, '科研工作量导入模板.xls')
      })
    },
    handleDownload() {
